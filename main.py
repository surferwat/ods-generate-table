...
class TableEntries:
  PATH_TO_SOURCE_FILES = "/test/screened"
  ...
  PROPERTY_NAME_CELL = 'B12'
  ADMIN_AREA_CELL = 'B24'
  DISTRICT_CELL = 'B13'
  PROPERTY_TYPE_CELL = 'B31'
  YEAR_BUILT_CELL = 'B32'
  TOTAL_AREA_CELL = 'B35'
  PRICE_CELL = 'B7'
  PRICE_PER_AREA_CELL = 'B6'
...
class Table:
  PATH_TO_TABLE_TEMPLATE = "test/test_table.ods"
  ...
  def generate(self):
    ...
    Returns:
      Nothing.
    """
    file_path = self.PATH_TO_TABLE_TEMPLATE
    self.check_file_path(file_path)
    self.table_sheet = self.get_data(file_path)
    row = self.FIRST_ROW
    ...
    self.table_sheet[self.FIRST_ROW - 2, self.HEADER_COL] = str(date.today().year) + "年" + str(date.today().month) + "月"
...
if __name__ == "__main__":
  ...
  table_entries.set_source_file_names(table_entries.PATH_TO_SOURCE_FILES)
